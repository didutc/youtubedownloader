...
def setURL(nextPage):
    ...
    pageCount += 1
    if nextPage != '':
        crafted_youtube_url = "https://www.googleapis.com/youtube/v3/search?key=" + API_KEY + "&channelId=" + \
            channel + "&pageToken=" + nextPage + \
            "&part=snippet&order=date&maxResults=" + str(SET_FETCH_LIMIT)
    else:
        crafted_youtube_url = "https://www.googleapis.com/youtube/v3/search?key=" + API_KEY + \
            "&channelId=" + channel + \
            "&part=snippet&order=date&maxResults=" + str(SET_FETCH_LIMIT)
...
def processUrl(uRL):
    ...
    requester.addheaders = [
        ('User-Agent', 'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/50.0.2311.135 Safari/537.36')]
...
def getImg(title, videoID, altUrl, count):
    ...
    try:
        resource = urlReq.urlopen(
            "https://i.ytimg.com/vi/" + videoID + "/sddefault.jpg")
    except:
        resource = urlReq.urlopen(altUrl)
    output = open("./thumbnails/" + title + "-" +
                  str(pageCount) + "-" + str(count) + ".jpg", "wb")
